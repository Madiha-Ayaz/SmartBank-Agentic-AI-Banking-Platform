...
# ─── Simple API Key Auth ───
API_KEY = os.getenv("BANKING_API_KEY", "sb-dev-key-2026")
security = HTTPBearer(auto_error=False)

async def verify_api_key(credentials: HTTPAuthorizationCredentials = Depends(security)):
    """Simple API key authentication for banking API."""
    if settings.DEBUG:
        return True  # Allow all in debug mode
...
# ─── Apply API Key auth globally to all routes ───
...
async def auth_middleware(request: Request, call_next):
    ...
    if not auth.startswith("Bearer ") and not settings.DEBUG:
        from fastapi.responses import JSONResponse
        return JSONResponse(status_code=401, content={"detail": "Missing API key"})
    token = auth.replace("Bearer ", "")
    if token != API_KEY and not settings.DEBUG:
        from fastapi.responses import JSONResponse
